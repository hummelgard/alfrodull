...
while True:
    ...
    for i in range(samples):
        ...
        shuntvoltageList.append( ina219.reverseByteOrder(ina219.readU16(
                                           INA219_REG_SHUNTVOLTAGE)) * 0.01)
        busvoltageList.append( 
          (ina219.readS16(INA219_REG_BUSVOLTAGE) >> 3) * 4 * 0.001) 
...
#        busvoltageList.append( 
#          (ina219.reverseByteOrder(ina219.readU16(
#          INA219_REG_BUSVOLTAGE)) >> 3) * 4 * 0.001 )    

#        shuntcurrentList.append( 
#          ina219.reverseByteOrder(ina219.readU16(
#          INA219_REG_CURRENT)) / ina219_currentDivider_mA )
        
        print("{0:6.3f}V, {1:4.1f}mA, {2:5.2f}mV".format(busvoltageList[-1],shuntcurrentList[-1],shuntvoltageList[-1],"mV"))
        time.sleep(0.2)

    busvoltage = sorted(busvoltageList)[samples//2]
    shuntvoltage = sorted(shuntvoltageList)[samples//2]
    shuntcurrent = sorted(shuntcurrentList)[samples//2]
